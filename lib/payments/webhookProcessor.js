...
import { applyFounderEntitlement } from "./founderEntitlements.js";
...
export async function processSquareWebhook({ firestore, environment, locationId, event, rawBody, now }) {
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    if (normalized.outcome === "ignored" && normalized.reason === "unrecognized_provider_order_id") {
      record.processingState = "retryable_unlinked";
    }
    if (normalized.outcome === "processed") {
      ...
      if (normalized.outcome === "processed") {
        ...
        if (normalized.outcome === "processed" && normalized.transaction.purpose === "founder") {
          await applyFounderEntitlement({ firestore, tx, environment, order: trustedOrder, transaction: normalized.transaction, transactionId: normalized.transactionId, timestamp });
        }
        if (normalized.outcome === "processed" && !transactionSnap.exists) {
