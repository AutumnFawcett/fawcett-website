import { FOUNDER_TIERS } from "./founderOffers.js";

export const FOUNDER_GOAL_CENTS = 5000000;
const environments = new Set(["sandbox", "production"]);
const statuses = new Set(["active", "inactive", "disputed_hold"]);
const tiers = FOUNDER_TIERS;
const integer = (value) => Number.isSafeInteger(value) && value >= 0;
const dateValue = (value) => value?.toDate?.() || (value instanceof Date ? value : null);

export function campaignProgressPercentage(amountCents, goalCents) {
  if (!Number.isSafeInteger(amountCents) || amountCents < 0
    || !Number.isSafeInteger(goalCents) || goalCents <= 0) return 0;
  return Math.min(100, Math.max(0, amountCents * 100 / goalCents));
}

export function formatCampaignPercentage(amountCents, goalCents) {
  const percentage = campaignProgressPercentage(amountCents, goalCents);
...
export function safeFounderProfile(profile, options) {
  ...
  const earned = tiers.find((tier) => tier.id === profile.earnedTierId) || null;
  ...
  return {
    ...
    earnedBenefits: earned ? [...earned.benefits] : [],
    hasPermanentFounderStatus: Boolean(earned), recognitionEligible: Boolean(earned),
    nextTier: next && { id: next.id, name: next.name, thresholdCents: next.thresholdCents,
      remainingCents: next.thresholdCents - profile.eligibleContributionCents,
      additionalBenefits: [...next.additionalBenefits],
...
export function campaignStats(profiles, environment) {
  ...
  return { goalAmountCents: FOUNDER_GOAL_CENTS, eligibleAmountCents, supporterCount,
    percentage: campaignProgressPercentage(eligibleAmountCents, FOUNDER_GOAL_CENTS) };
}
