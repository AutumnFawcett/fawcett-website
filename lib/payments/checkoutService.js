...
const ALLOWED_INPUTS = new Set(["offerId", "acknowledgement"]);
...
function assertReservedOrder(reservation, offer, requestId, clientUid, environment, acknowledgementVersion) {
  ...
  if (!mapping || mapping.requestId !== requestId || typeof order?.orderId !== "string" || !order.orderId
    ...
    || order.acknowledgementVersion !== acknowledgementVersion || timestampMs(order.acknowledgedAt) === null
    || timestampMs(order.acknowledgedAt) !== timestampMs(order.createdAt)
  ...
  if (order.status === "creating") {
    ...
    if (typeof order.providerOrderId !== "string" || !order.providerOrderId.trim()
      ...
      || !isSafeSquareCheckoutUrl(order.checkoutUrl, environment) || order.failureCode !== null) fail();
...
function assertRecoverableOrder(order, offer, requestedOrderId, environment, acknowledgementVersion) {
...
function assertAcknowledgement(input) {
  if (!input?.acknowledgement || Object.getPrototypeOf(input.acknowledgement) !== Object.prototype
    || Object.keys(input.acknowledgement).length !== 1 || input.acknowledgement.accepted !== true) throw new Error("acknowledgement_required");
}

async function createAndAttach({ order, offer, config, storage, provider, now }) {
  let response;
  try {
    response = await provider.createPaymentLink({ order, offer, locationId: config.locationId, redirectUrl: config.redirectUrl });
  } catch {
    throw reconciliationRequired(order);
  }
  let identity;
  try {
    identity = validatePaymentLinkResponse(response, order, config.environment);
  } catch {
...
export async function createIdempotentFounderCheckout({ input, requestId, clientUid, config, storage, provider, now = () => new Date(), createId = () => randomUUID().replaceAll("-", "") }) {
  ...
  const order = {
    ...
    checkoutUrl: null, idempotencyKey: `fawcett-${orderId}`, acknowledgementVersion: config.acknowledgementVersion,
    acknowledgedAt: timestamp, createdAt: timestamp,
  ...
  const reservedOrder = assertReservedOrder(reservation, offer, requestId, clientUid, config.environment, config.acknowledgementVersion);
  if (reservedOrder.status === "pending") return reservedOrder;
  assertRecoverableOrder(reservedOrder, offer, reservedOrder.orderId, config.environment, config.acknowledgementVersion);
...
export async function createFounderCheckout({ input, clientUid = null, config, storage, provider, now = () => new Date(), createId = () => randomUUID().replaceAll("-", "") }) {
  ...
  const order = {
    ...
    acknowledgementVersion: config.acknowledgementVersion, acknowledgedAt: timestamp,
...
export async function reconcileFounderCheckout({ orderId, config, storage, provider, now = () => new Date() }) {
  ...
  assertRecoverableOrder(order, offer, orderId, config.environment, config.acknowledgementVersion);
  return createAndAttach({ order, offer, config, storage, provider, now });
}
