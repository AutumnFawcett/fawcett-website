...
function safeResult(checkout, environment) {
  ...
  return isSafeSquareCheckoutUrl(checkout.checkoutUrl, environment);
}
...
export async function handleFounderCheckoutRequest(request, dependencies) {
  ...
  if (!body || Object.getPrototypeOf(body) !== Object.prototype || Object.keys(body).length !== 3 || Object.keys(body).some((key) => !["offerId", "requestId", "acknowledgement"].includes(key)) || typeof body.requestId !== "string" || !FOUNDER_REQUEST_ID_PATTERN.test(body.requestId) || typeof body.offerId !== "string"
    || !body.acknowledgement || Object.getPrototypeOf(body.acknowledgement) !== Object.prototype
    || Object.keys(body.acknowledgement).length !== 1 || body.acknowledgement.accepted !== true) return json({ error: "invalid_request" }, 400);
  let config;
  ...
  try {
    const checkout = await dependencies.checkout({ input: { offerId: body.offerId, acknowledgement: { accepted: true } }, requestId: body.requestId, clientUid, config, ...(await dependencies.getCheckoutDependencies()) });
    if (!safeResult(checkout, config.environment)) throw new Error("unsafe_checkout_result");
