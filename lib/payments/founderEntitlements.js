...
function assertTrustedProfile(profile, { environment, clientUid }) {
  const recognitionIsValid = (profile.recognitionMode === "anonymous" && profile.publicRecognitionEnabled === false)
    || (profile.recognitionMode === "public" && profile.publicRecognitionEnabled === true);
  const aggregatesAreValid = [
    "confirmedContributionCents", "refundedContributionCents",
    "disputedContributionCents", "eligibleContributionCents",
  ].every((field) => Number.isSafeInteger(profile[field]) && profile[field] >= 0);
  if (profile.clientUid !== clientUid || profile.environment !== environment
    || profile.currency !== "CAD" || !recognitionIsValid || !aggregatesAreValid
    || !Number.isSafeInteger(profile.founderNumber) || profile.founderNumber < 1
    || !["active", "inactive", "disputed_hold"].includes(profile.status)) {
    throw new Error("founder_profile_collision");
  }
}

function assertSafeAggregates(aggregates) {
  if (!Object.values(aggregates).every((value) => Number.isSafeInteger(value) && value >= 0)) {
    throw new Error("founder_aggregate_overflow");
  }
}
...
export async function applyFounderEntitlement({ firestore, tx, environment, order, transaction, transactionId, timestamp }) {
  ...
  if (contributionSnap.exists) {
    ...
    if (!profileSnap.exists) throw new Error("founder_profile_missing_for_contribution");
    assertTrustedProfile(profileSnap.data(), { environment, clientUid: transaction.clientUid });
    return { applied: false, duplicate: true, profileId, contributionId };
  }
  const profile = profileSnap.exists ? profileSnap.data() : null;
  if (profile) assertTrustedProfile(profile, { environment, clientUid: transaction.clientUid });
  ...
  assertSafeAggregates({ confirmed, refunded, disputed, eligible });
  const tier = earnedFounderTier(eligible);
  const updated = {
    clientUid: transaction.clientUid, environment, founderNumber,
    recognitionMode: profile ? profile.recognitionMode : "anonymous",
    publicRecognitionEnabled: profile ? profile.publicRecognitionEnabled : false,
    currency: "CAD", confirmedContributionCents: confirmed,
