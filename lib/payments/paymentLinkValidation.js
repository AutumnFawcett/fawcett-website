...
import { isSafeSquareCheckoutUrl, isSquareCheckoutHost } from "./founderCheckoutValidation.js";
...
export function validatePaymentLinkResponse(response, order, environment) {
  ...
  if (!isSquareCheckoutHost(parsed.hostname, environment)) throw new Error("invalid_checkout_host");
  if (!isSafeSquareCheckoutUrl(checkoutUrl, environment)) throw new Error("invalid_checkout_url");
