...
test("supported linked payment creates a CAD cents transaction", () => {
  const result = normalizeSquareEvent(paymentEvent(), "sandbox");
...
test("unlinked POS payments are ignored", () => assert.equal(normalizeSquareEvent(paymentEvent({ reference_id: undefined }), "sandbox").outcome, "ignored"));
test("amount and currency mismatches are rejected", () => {
  assert.throws(() => normalizeSquareEvent(paymentEvent({ amount_money: { amount: 2499, currency: "CAD" } }), "sandbox"), /amount_mismatch/);
  assert.throws(() => normalizeSquareEvent(paymentEvent({ amount_money: { amount: 1.5, currency: "CAD" } }), "sandbox"), /invalid_amount/);
  assert.throws(() => normalizeSquareEvent(paymentEvent({ amount_money: { amount: 1, currency: "USD" } }), "sandbox"), /invalid_currency/);
});
test("refunds and chargebacks are separate linked records", () => {
  const parent = { purpose: "founders", orderId: "order" };
  const refund = normalizeSquareEvent({ event_id: "r", type: "refund.updated", data: { object: { refund: { id: "refund", payment_id: "pay", amount_money: { amount: 500, currency: "CAD" }, status: "COMPLETED" } } } }, "sandbox", parent);
  const dispute = normalizeSquareEvent({ event_id: "d", type: "dispute.updated", data: { object: { dispute: { id: "dispute", disputed_payment: { payment_id: "pay" }, amount_money: { amount: 2500, currency: "CAD" }, state: "LOST" } } } }, "sandbox", parent);
...
test("unknown events are ignored", () => assert.equal(normalizeSquareEvent({ type: "catalog.version.updated" }, "sandbox").outcome, "ignored"));
