...
## Authenticated Founder checkout

The Founder page renders its checkout controls from the server-only `SQUARE_PAYMENTS_ENABLED` value. With the committed value `false`, every tier remains non-clickable and says that support is available at launch. A separately approved Sandbox runtime may set the flag to exactly `true`; this is only a presentation gate, while the API independently enforces the same kill switch.

An enabled browser obtains the current Firebase user's ID token and posts only `{ offerId, requestId }` as JSON to `POST /api/payments/checkout/founder`. The API verifies `Authorization: Bearer <token>` with Firebase Admin and revocation checking. The verified `uid` is the sole identity authority; body identity, amount, currency, quantity, descriptions, order IDs, URLs, and every other unexpected field are rejected. A signed-out user is sent through the existing Tattoo Portal login with the return destination restricted to `/founders`.

The `requestId` is a 16–64 character non-commercial retry token. A Firestore transaction creates `paymentCheckoutRequests/{sha256(provider:environment:requestId)}` and the server-generated `paymentOrders` document together. The request record binds the token to UID, offer, provider, environment, and order ID. Sequential and concurrent retries reload that order and reuse its stable Square idempotency key; reuse by another UID or offer is a conflict. A durable success returns the stored validated result without making another Payment Link. Ambiguous provider or persistence outcomes retain the creating order for reconciliation and never return a URL.

The same reservation transaction enforces a conservative limit of five new checkout requests per verified UID per hour in `paymentCheckoutRateLimits`; an existing request is reconciled before this check and therefore cannot be stranded by the limit. Both server-owned collections are explicitly unreadable and unwritable by unauthenticated users, ordinary users, and browser admins.

Responses use `Cache-Control: no-store` and expose only stable safe errors. Success contains only `orderId` and a validated Square-hosted `checkoutUrl`. Authentication failure is `authentication_required` (401), invalid input is `invalid_request` (400/415), token reuse conflict is `request_conflict` (409), rate limiting is `rate_limited` (429), disabled checkout is `payments_disabled` (503), and ambiguous/persistence states are `provider_outcome_unknown` or `reconciliation_pending` (503). Provider, Firebase, Firestore, configuration, and bearer-token details are never returned.

The browser redirects only after a successful safe response. A browser URL, return/cancel navigation, query string, or client state is never payment evidence and cannot update an order or create a transaction. Payment confirmation remains exclusively controlled by a verified Square webhook.

For an approved Sandbox test, use an isolated Firebase project, Sandbox Square credentials, and a test account; enable the flag only in that isolated runtime, sign in through the Tattoo Portal, select each fixed offer, and verify the resulting Square-hosted URL and webhook reconciliation. Exercise duplicate and ambiguous requests with the same retry token, confirm one internal order and stable Square idempotency key, and inspect creating orders before retrying reconciliation. Keep `SQUARE_PAYMENTS_ENABLED=false` everywhere else pending separate approval, and never test against production services or promote the preview.
